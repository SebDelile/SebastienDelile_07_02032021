...
// template Li element : <li class="searchedtag__list__item"><button class="searchedtag__list__button">item 1</button></li>
...
export function tagsImport(recipe) {
  for (let item of recipe.ingredients) {
    if (!listIngredients.includesData("shown", item.ingredient)) {
      listIngredients.insertAtBeginning("shown", norm(item.ingredient));
  ...
  if (recipe.appliance && !listAppliances.includesData("shown", recipe.appliance)) {
    listAppliances.insertAtBeginning("shown", norm(recipe.appliance));
  ...
  for (let item of recipe.ustensils) {
    if (!listUstensils.includesData("shown", item)) {
      listUstensils.insertAtBeginning("shown", norm(item));
